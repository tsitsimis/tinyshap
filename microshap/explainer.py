...
class SHAPExplainer:
    ...
    def _generate_feature_combinations(self, x_test: np.ndarray, feature_name: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        mask_df = pd.DataFrame(
            np.random.choice([0, 1], size=(self.n_samples, dim-1)),
    ...
    def _explain_single_feature(self, x_test: np.ndarray, feature_name: str) -> float:
        ...
        marginal_contributions = (
            self.model(combinations_with_feature) - self.model(combinations_without_feature)
        )
